...
class Generate
{
    ...
    {
        return deleteFile($pathView . 'index.html');
    }
    ...
    {
        return deleteFile($pathView . 'form.html');
    }
    ...
    {
        return deleteFile($pathView . 'th.html');
    }
    ...
    {
        return deleteFile($pathView . 'td.html');
    }
    ...
    {
        return deleteFile($pathView . 'edit.html');
    }
    ...
    {
        return deleteFile($pathView . 'recyclebin.html');
    }
    ...
    {
        return deleteFile($pathView . 'config.php');
    }
    ...
    private function buildEdit($path, $pathTemplate, $fileName, $tableName, $code, $data)
    {
        ...
        return file_put_contents($file, str_replace(
            ["[MODULE]", "[ROWS]", "[SET_VALUE]", "[SCRIPT]", "[STYLE]"],
            [$module, $code['edit'], implode("\n", array_merge($code['set_checked'], $code['set_selected'], $code['set_ueditor'])), implode("", $code['script_edit']), implode("", $code['style_edit'])],
            $template));
    }
                    ...
                    switch ($form['type']) {
                        ...
                        case "checkbox":
                            if ($form['type'] == "radio") {
                                ...
                                $setChecked[] = tab(2) . 'var checks = \'{$vo.' . $form['name'] . ' ?? \'' . $form['default'] . '\'}\'.split(",");' . "\n"
                        ...
                        case "date":
                            ...
                            break;
                        case "file":
                            $editField .= tab(4) . '<input type="text" class="input-text" '
                                . 'placeholder="请上传' . $form['title'] . '" name="' . $form['name'] . '" id="' . $form['name'] . '" style="width:70%"'
                                . 'value="' . '{$vo.' . $form['name'] . ' ?? \'' . $form['default'] . '\'}' . '" '
                                . $validateForm . '>' . "\n"
                                . tab(4) . '<button type="button" class="btn btn-primary radius" onclick="layer_open(\'文件上传\',\'{:\\\\think\\\\Url::build(\\\'Upload/index\\\', [\\\'id\\\' => \\\'' . $form['name'] . '\\\'])}\')">上传</button>' . "\n"
                                . tab(4) . '<a onclick="$(this).attr(\'href\', $(\'#' . $form['name'] . '\').val())" type="button" class="btn btn-success radius" data-lightbox="preview">预览</a>' . "\n";
                            $scriptEdit['lightbox'] = "\n" . '<script src="__LIB__/lightbox2/js/lightbox.min.js"></script>';
                            $styleEdit['lightbox'] = "\n" . '<link rel="stylesheet" href="__LIB__/lightbox2/css/lightbox.min.css">';
                            break;
                        case "richtext":
                            $editField .= tab(4) . '
                              <textarea id="' . $form['name'] . '" name="' . $form['name'] . '" type="text/plain" style="height:400px">' . '{$vo.' . $form['name'] . ' ?? \'' . $form['default'] . '\'}' . '</textarea>' . "\n";
                            $setUeditor[] =  tab(2) . 'var ue = UE.getEditor(\'' . $form['name'] . '\',{' . "\n" .
                            tab(3) . 'serverUrl:\'{:\\\\think\\\\Url::build("Ueditor/index")}\'' . "\n" .
                            tab(2) . '});';
