 ...
 * @param texts
 */
function change_status(ret, obj, type, field, texts) {
    console.log('in'+texts);
    //配置数据，TYPE:['下一状态文字描述','当前状态class颜色','下一状态class颜色','下一状态方法名','状态标签选择器','下一状态标签icon','下一状态标签title']
    texts = texts ? texts.split(',') : ['禁用','恢复'];
    field = field ? field : 'status';
    var data = {
        'resume': [texts[1], 'success', 'warning', 'forbid', '.'+field, '&#xe615;', '正常'],
        'forbid': [texts[0], 'warning', 'success', 'resume', '.'+field, '&#xe631;', '禁用']
    };
    var $this = $(obj);
