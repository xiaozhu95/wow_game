...
//            }
    ...
    public function orderList($data)
    {
        ...
        $confirm_payment_list_pay = model('confirm_payment')->alias('payment')
            ...
            ->join('auction_log log','payment.auction_log_id = log.id','left')
            ->join('auction_pay pay','payment.id = pay.confirm_payment_id','left')
            ->field('e.id,e.team_id,e.boss_id,e.type,e.user_id,e.equipment_id,e.equipment_name,e.currency_type,log.price,e.finsih_after_time,e.pay_after_time,e.end_time,e.pay_end_time,payment.id as confirm_payment_id,log.id as auction_log_id,pay.confirm_status,pay.pay_type')
            ->where('payment.user_id = e.user_id')
        ...
        //$role = model('role');
        //$role_info =  $role->where(['id'=>$team_info['role_id']])->find();
        $team_ids = array_column($confirm_payment_list, 'team_id');
        ...
        foreach ($confirm_payment_list as $key => $value) {
            ...
            $confirm_payment_list[$key]['user'] = isset($user[$value['user_id']]) ? $user[$value['user_id']]   : [];
          	if( $confirm_payment_list[$key]['user']){
              	 ...
              	 $confirm_payment_list[$key]['user']['nickname'] = $this->filterRole($team_members_info, $value['user_id'], $value['team_id']);;
            }
          	$is_team_log = 0;
            if(isset($team['user_id']) && $team['user_id'] == $value['user_id'] ){
                 $is_team_log = 1;
