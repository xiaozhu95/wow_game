...
class AuctionPay extends Model
{
    ...
    const CONFIRM_STATUS_REFUSE = 3; //团长审核拒绝（金币审核不通过）
    ...
    public function apyList ($data)
    {
        ...
        $currencyCoin = $this->where(["pay_type" => AuctionPay::PAY_TYPE_YES])->where(["team_id" => $data["team_id"]])->where(["currency_type" => AuctionPay::CURRENCY_TYPE_GLOD])->select()->toArray();
        $currencyMoney = $this->where(["pay_type" => AuctionPay::PAY_TYPE_YES])->where(["team_id" => $data["team_id"]])->where(["currency_type" => AuctionPay::CURRENCY_TYPE_MONEY])->select()->toArray();

        foreach ($currencyMoney as $currencyMoneyKey => $currencyMoneyValue) {
            if ($currencyMoneyValue['equipment_id'] == 0) {
                $currencyMoney[$currencyMoneyKey]['equipment_name'] = "拍卖地板";
            }
        }

        foreach ($currencyCoin as $currencyCoinKey => $currencyCoinValue) {
            if ($currencyCoinValue['equipment_id'] == 0) {
                $currencyCoin[$currencyCoinKey]['equipment_name'] = "拍卖地板";
            }
        }
        $result = [
            'code' => 0,
        ...
        if($data['equipment_id'] == 0){ //拍卖地板
            ...
            if($data['currency_type'] == self::CURRENCY_TYPE_MONEY){ //用钱购买
                if($user['balance']<$data['price']){
                    return ajax_return_adv_error('你的余额不足,请充值');
                }
                //扣钱
...
//            }
            ...
            $auctionPayDaya['confirm_payment_id'] = $data['auction_equipment_id'];
            ...
            if($auction_pay){
                ...
                $auction_log->is_winner = 1;
                $auction_log_result = $auction_log->save();
                //修改准备表信息
                $actionResult =$auctionEquipmentModel->where(['id'=>$auction_log['auction_equipment_id'],'team_id'=>$data['team_id'],'type'=>AuctionEquipment::TYPE_IN_TRANSACTION])
                    ->update(['type'=>$type,'user_id'=>$data['user_id'],'pay_money'=> $auctionPayDaya['price'],'pay_time'=>time(),'pay_role_id'=>$auction_log['role_id'],'pay_role_name'=>$auction_log['role_name']]);
                if($actionResult  && $auctionPayResult && $auction_log_result){
    ...
    public function review($order_id)
    {
        ...
        if ($auction_pay){
            ...
            if ($auction_pay['equipment_id'] == 0){ //成为地板
                ...
                $auction_equipment_result = model('auction_equipment')->where(['id'=>$auction_pay['confirm_payment_id']])->update(['type'=> AuctionEquipment::TYPE_SUCCESSFUL_TRANSACTION]);
    ...
    public function refuse($order_id)
    {
        ...
        $auction_pay = model('auction_pay')->where(['id'=>$order_id,'pay_type'=> self::PAY_TYPE_YES,'currency_type'=> self::CURRENCY_TYPE_GLOD])->find();
        if($auction_pay){
            $auction_pay->confirm_status = self::CONFIRM_STATUS_REFUSE;

            $confirm_payment = model('confirm_payment')->field('auction_equipment_id')->where(['id'=> $auction_pay->confirm_payment_id ])->find();
            $auction_result = 0;
            if ($confirm_payment){
                $auction_equipment_id = $confirm_payment['auction_equipment_id'];
                $auction_result = model('auction_equipment') ->where(['id'=>$auction_equipment_id])->update(['type'=>2,'user_id'=>0]);
            }

            $pay = $auction_pay->save();

            if ($pay && $auction_result){
