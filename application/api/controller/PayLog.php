...
//		]
        4 => [ // 小程序 动力工厂
            'app_id' => 'wxa07d866227c5c901', // APPID (开户邮件中可查看)
            'mch_id' => '1525967161', // 商户号 (开户邮件中可查看)
            'app_key' => 'dongligongchangdongligongchang12' // 商户支付密钥 (https://pay.weixin.qq.com/index.php/account/api_cert)
        ]
    ];
    ...
    public function add()
    {
        ...
        if($subjects[$id] && $this->pay_types[$type] && $user_id){
            $total_amount = $this->request->param('price');
    ...
    public function payToUser()
    {
        ...
        $total_price = decrypt($total_price,$vi);
        $code = decrypt($code,$vi);

        if(!$user_id || !$mobile || !$total_price || !$code) return ajax_return_adv_error('用户信息解密有误');

        if($token == md5($user_id)){
            $deviant = model('deviant')->where(['user_id'=>$user_id])->find();
            if($deviant)  return ajax_return_adv_error('账户信息异常,无法提现，请联系客服');
            $result =  $this->smsVery($mobile,$code);

            $flag = json_decode($result->getContent(),true);

            if ($flag['code']) return $result;

            Db::startTrans();
            $user_model = model('user');
            $user = $user_model->where(['id'=>$user_id])->find();
            if(!$user) return ajax_return_adv_error('用户不存在');
            if($user->balance - $total_price < 0){
                return ajax_return_adv_error('余额不足,无法提现');
            }

            require_once(ROOT_PATH.'sdks'.DS.'wechat_pay'.DS.'WechatPay.php');
            $configs = [
                'app_id' => 'wxa07d866227c5c901', // 商户账号appid
                'mch_id' => '1525967161', //商户号
                'app_key' => 'dongligongchangdongligongchang12', //支付秘钥
            ];
            $pay = new \WechatPay($configs);
            $order_no =  time();
            $data['withdraw_ids'] =$order_no; // 商户订单号
            $data['openid'] = $user['openid'] ; //用户openid
            $data['amount'] = $total_price * 100; //金额
            $data['desc'] = "用户提现"; // 企业付款备注
            ...
            if($user_result && $result['result_code'] == "SUCCESS" ){
                $user_money_log_result = model('UserMoneyLog')->data([
                    'user_id' => $user_id,
                    'amount' => - $total_price,
                    'type' => 0,
                    'msg' => '提现',
                ...
                if ($user_money_log_result){
                    Db::commit();;
                    return ajax_return(['balance'=>$user->balance],"提现成功");
                }
            }
            DB::rollback();
            return ajax_return_adv_error('提现失败,请重试');
        }
        return ajax_return_adv_error('验证有误');
    }
    /**
     * 支付验证码验证
     * 验证
    ...
    public function smsVery($mobile,$code)
    {
        ...
        //判断是否是用户名登陆
        $smsStatus = $model->check($mobile, $code, 'pay');
        if($smsStatus == 1){
            $result['msg'] = '短信验证码错误';
            return json($result);
        }elseif($smsStatus==2){
