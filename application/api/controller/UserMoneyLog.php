...
class AuctionPay extends Model
{
    ...
    public function apy($data)
    {
        ...
        if($data['equipment_id'] == 0){ //拍卖地板
              Db::startTrans();
           $diban = model('auction_pay')->where(['team_id'=>$data['team_id'],'equipment_id'=> $data['equipment_id']])->find();
    ...
    public function review($order_id)
    {
       ...
       if ($auction_pay){
           ...
           $auction_pay->confirm_status = 2;
           $auction_pay_result = $auction_pay->save();
           $result  = $team->save();

           if($result && $team_member && $auction_pay_result){
