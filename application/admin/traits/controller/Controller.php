...
trait Controller
{
    ...
    public function add()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            return ajax_return_adv('添加成功','parent','',false,'',$model->getData());
        } else {
            // 添加
    ...
    public function edit()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            if (!$data[$this->getModel()->getPk()]) {
                return ajax_return_adv_error("缺少主键ID");
            }
            ...
            if (
                ...
                $model = new $modelClass();
                $ret = $model->isUpdate(true)->allowField(true)->save($data, [$this->getModel()->getPk() => $data[$this->getModel()->getPk()]]);
            } else {
                // 简单的直接使用db更新
                Db::startTrans();
                try {
                    $model = Db::name($this->parseTable($controller));
                    $ret = $model->where($this->getModel()->getPk(), $data[$this->getModel()->getPk()])->update($data);
                    // 提交事务
                    Db::commit();
                } catch (\Exception $e) {
                    // 回滚事务
            ...
            // 编辑
            $id = $this->request->param($this->getModel()->getPk());
            if (!$id) {
                throw new HttpException(404, "缺少主键ID");
            }
    ...
    public function setField()
    {
        ...
        return $this->updateField($data['field'], $data[$data['field']] , "设置成功");
    }
