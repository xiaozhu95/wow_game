 ...
 * @param int $id            对象id
 * @param int $where_field   主键字段
 * @param string $field      需切换的字段
 * @param string $controller 默认当前控制器
 * @return string
 */
function show_status($status, $id, $where_field = 'id', $field = 'status', $controller = '', $texts = '')
{
    $controller === '' && $controller = Request::instance()->controller();
    $texts = $texts ?: '恢复,禁用,还原';
    $texts_arr = explode(',',$texts);
    switch ($status) {
        // 恢复
        case 0 :
            //老版本可能有覆写 resume/forbid 做一些操作 所以当是status的时候 还是采用默认的 其他的采用setField
            $url = $field == 'status' ? Url::build($controller . '/resume', [$where_field => $id]) : Url::build($controller . '/setField', [$where_field => $id, 'field' => $field, $field => 1]);
            $ret = '<a href="javascript:;" onclick="ajax_req(\'' . $url . '\',{},change_status,[this,\'resume\',\''.$field.'\',\''.$texts.'\'])" class="label label-success radius" title="点击'.$texts_arr[0].'">'.$texts_arr[0].'</a>';
            break;
        // 禁用
        case 1 :
            $url = $field == 'status' ? Url::build($controller . '/forbid', [$where_field => $id]) : Url::build($controller . '/setField', [$where_field => $id, 'field' => $field, $field => 0]);
            $ret = '<a href="javascript:;" onclick="ajax_req(\'' . $url . '\',{},change_status,[this,\'forbid\',\''.$field.'\',\''.$texts.'\'])" class="label label-warning radius" title="点击'.$texts_arr[1].'">'.$texts_arr[1].'</a>';
            break;
        // 还原
        case -1 :
            $ret = '<a href="javascript:;" onclick="ajax_req(\'' . Url::build($controller . '/recycle', [$where_field => $id]) . '\')" class="label label-secondary radius" title="点击'.$texts_arr[2].'">'.$texts_arr[2].'</a>';
            break;
    }
 ...
 * @return string
 */
function get_status($status, $imageShow = true, $className = 'status')
{
    switch ($status) {
        case 0 :
            $showText = '禁用';
            $showImg = '<i class="Hui-iconfont c-warning'.($className ? " ".$className : "").'" title="禁用">&#xe631;</i>';
            break;
        case -1 :
            $showText = '删除';
