        ...
        foreach ($menu as $k => $m) {
            ...
            switch ($m) {
                ...
                case 'sedit':
                    ...
                    list($url, $param) = $this->parseUrl($url, $pk.'=$vo["'.$pk.'"]');
                ...
                case 'sdelete':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '删除';
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= ' <a title="' . $title . '" href="javascript:;" onclick="del(this,\'{$vo.'.$pk.'}\',\'<?php echo \think\Url::build(\'' . $url . '\', [' . $param . ']); ?>\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>';
                    break;
                case 'srecycle':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '还原';
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= ' <a href="javascript:;" onclick="recycle(this,\'{$vo.'.$pk.'}\',\'<?php echo \think\Url::build(\'' . $url . '\', [' . $param . ']); ?>\')" class="label label-success radius ml-5">' . $title . '</a>';
                    break;
                case 'sdeleteforever':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '彻底删除';
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= ' <a href="javascript:;" onclick="del_forever(this,\'{$vo.'.$pk.'}\',\'<?php echo \think\Url::build(\'' . $url . '\', [' . $param . ']); ?>\')" class="label label-danger radius ml-5">' . $title . '</a>';
                    break;
                default:
                    // 默认为小菜单
