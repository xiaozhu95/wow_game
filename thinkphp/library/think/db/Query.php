...
class Query
{
    ...
    public function paginate($listRows = null, $simple = false, $config = [])
    {
        ...
        if (!isset($total) && !$simple) {
            ...
            //paginate + cache 的链式操作出错。。。。 因为paginate里面有个计算总数的count 而这个count 和 实际查询列表 公用了一个缓存key。。。。 如果指定了key的话  先自己解决了
			if(!empty($options['cache']) && is_string($options['cache']['key']))
            $options['cache']['key'].="list";
